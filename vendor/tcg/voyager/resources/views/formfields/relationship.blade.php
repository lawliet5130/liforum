    ...
    @if(class_exists($options->model))
        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @if(isset($query))
                    <p>{{ $query->{$options->label} }} @if($options->table=="scientist_accounts") {{$query->surname}} @endif</p>
                @else
                    <p>No results</p>
                @endif
            ...
            @else
                ...
                <select class="form-control select2" name="{{ $options->column }}">
                    ...
                    @foreach($query as $relationshipData)
                        <option value="{{ $relationshipData->{$options->key} }}" @if($dataTypeContent->{$options->column} == $relationshipData->{$options->key}){{ 'selected="selected"' }}@endif>{{ $relationshipData->{$options->label} }} @if($options->table=="scientist_accounts") {{$relationshipData->surname}} @endif</option>
                    @endforeach
                </select>
